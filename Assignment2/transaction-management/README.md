...
A production-quality Next.js application demonstrating authenticated,
role-based transaction management with a full audit trail, transactional email
and webhook-backed delivery tracking.
...
The system manages financial transactions for multiple users under three
roles — **ADMIN**, **MEMBER** and **GUEST**. Every privileged action is
...
**Guest**
- Limited dashboard with account details and an explanation of guest access
- No access to transaction data or admin routes
...
Enums: `UserRole` (ADMIN/MEMBER/GUEST), `TransactionType` (CREDIT/DEBIT),
...
**GUEST** and must be upgraded by an administrator.
...
| --- | :---: | :---: | :---: |
| `/dashboard` | limited | own data | all data |
...
**GUEST** — `/dashboard` shows the limited view; `/transactions` and `/admin`
redirect away; `/api/transactions` returns 403.
